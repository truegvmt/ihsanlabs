...
serve(async (req) => {
  ...
  // ── Step 3: Generate query embedding + RAG retrieval ──────────
  const embeddingInput = `${focus} ${body.three_words ?? ""}`.trim();
  // Embed the donor's intention to find the most relevant evidence chunks
  let queryEmbedding: number[] = new Array(1536).fill(0); // safe fallback if OpenAI unavailable
  try {
    const embRes = await fetch("https://api.openai.com/v1/embeddings", {
      ...
      body: JSON.stringify({ input: embeddingInput, model: "text-embedding-3-small" }),
    });
    const embData = await embRes.json();
    queryEmbedding = embData.data?.[0]?.embedding ?? queryEmbedding;
  } catch (e) {
    console.error("[allocation-optimizer] Embedding call failed, using zero-vector fallback:", e);
  }
  ...
  // Guard: if no evidence chunks found, skip LLM and return deterministic-only plan
  if (retrievedChunks.length === 0) {
    console.log(JSON.stringify({ action: "rag_miss", focus, region, top_project_id: top.id }));
    // llmOutput already has deterministic fallback text — skip Anthropic call
  }

  // ── Step 4: LLM evocative layer (skipped if no RAG evidence) ──
  ...
  try {
    if (retrievedChunks.length === 0) throw new Error("No RAG chunks; using deterministic fallback");
